...
class GroundingVerifier:
    ...
    def on_source_probability(self, claim: str, passage: str) -> float:
        """Probability the passage supports the claim at least partially
        (1 - P(unsupported)). Unlike `predict`, this doesn't force a
        supported/partial call - which matters for LLM paraphrases, where the
        classifier (trained on near-verbatim pairs) reliably separates
        "on-source" from "made up" but not "faithful paraphrase" from "partial"."""
        if claim.strip() and claim.strip().lower() in passage.lower():
            return 1.0
        features = [extract_features(claim, passage)]
        probabilities = dict(
            zip(self.pipeline.classes_, self.pipeline.predict_proba(features)[0], strict=True)
        )
        return float(1.0 - probabilities.get("unsupported", 0.0))

...
class Topic(Base):
    """A literature-review request: a topic string, the papers found for it,
    and the cross-paper synthesis computed once they are all analyzed."""

    __tablename__ = "topics"

    id: Mapped[str] = mapped_column(String, primary_key=True, default=_uuid)
    query: Mapped[str] = mapped_column(String, nullable=False)
    # searching -> ingesting -> analyzing -> synthesizing -> done | error
    ...
    papers: Mapped[list["TopicPaper"]] = relationship(
        back_populates="topic", cascade="all, delete-orphan", order_by="TopicPaper.position"
    )
...
class TopicPaper(Base):
    ...
    published: Mapped[str] = mapped_column(String, nullable=False, default="")  # ISO date
    abstract: Mapped[str] = mapped_column(Text, nullable=False, default="")
    # pending -> ingesting -> analyzing -> done | error
    status: Mapped[str] = mapped_column(String, nullable=False, default="pending")
    error: Mapped[str | None] = mapped_column(Text, nullable=True)
    document_id: Mapped[str | None] = mapped_column(ForeignKey("documents.id"), nullable=True)
    # How the brief was written: "extractive" or "llm:<provider>".
    brief_mode: Mapped[str | None] = mapped_column(String, nullable=True)

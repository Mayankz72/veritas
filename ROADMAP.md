...
- [x] Deploy: done — see Phase 10 below for the full account setup, a real hosting-tier limitation found, and the honest conclusion.
...
**Deliverable:** ✅ 51 backend tests (41 unit + 10 integration, all passing), 9 frontend unit tests, 1 full-flow E2E test — all passing for real, verified in this session, not just written. Root `README.md` rewritten as the actual resume-facing artifact.

---

## Phase 10 — Deployment (done, with one disclosed limitation)
**Goal:** Get this live, at zero cost.

- [x] **GitHub**: pushed to `github.com/Mayankz72/veritas`. Commit history was rewritten with `git filter-branch` before the first push to strip `Co-Authored-By: Claude` trailers per the user's request — author/committer identity on every commit was already the user's own, confirmed via `git log --format` and via the GitHub API after push (`co-authors=false` on all 10 commits).
- [x] **Neon Postgres** (`veritas_paper_studio` project): linked via the Neon CLI's newer declarative tooling (`neon link`, `neon config init` → `neon.ts`, `neon deploy`). Two steps from the user's original instructions were skipped as non-essential and explained rather than forced: `neon skills` (needs Node 22.20+, this machine has 22.16) and `neon mcp` (needs a personal API key; the one available was org-scoped and can't mint new keys). Verified live: Postgres 18.6 reachable, `vector` extension available, `init_db()` created all 5 tables against the real database.
- [x] **Railway → blocked → Render**: Railway (the original pick) required a payment method even for its free allowance; switched to Render, which has a genuinely free web-service tier. Render has no CLI for *creating* new services (dashboard/API-driven only), so the service was created via Render's REST API directly (`POST /v1/services` with `env: docker`, pointing at `ml-service/Dockerfile` in the GitHub repo) — verified against the API's own schema (fetched an existing unrelated service first to confirm the exact JSON shape rather than guessing) before creating anything.
- [x] **Vercel**: linked and deployed to production via CLI (`vercel link`, `vercel env add`, `vercel --prod`), gated behind an explicit user confirmation before the `--prod` step per the deploy skill's own safety rule. Live at `app-woad-nu-48.vercel.app`.
- [x] **CORS**: the localhost-only regex from local dev needed extending for the real Vercel hostname(s) — updated to match any `*.vercel.app` subdomain (covers both production and preview deploys) plus localhost, and unit-verified against real and adversarial origins before deploying.

**Bug found via deployment, not local testing:** `PostgreSQL text fields cannot contain NUL (0x00) bytes` doesn't apply here (that was Phase 7) — this one was a genuine **OOM crash** on Render's free tier (512MB RAM / 0.1 shared vCPU): a 1-chunk document ingested fine, a real 58-chunk paper died with no Python traceback (the signature of a SIGKILL). First hypothesis: `fastembed`'s default `batch_size=256` runs a whole paper through the ONNX model as one batch. Added `EMBEDDING_BATCH_SIZE` (default 8, overridable) to bound peak memory - this is a real, permanent code improvement regardless of hosting, and it's what let a full paper ingest successfully at all (twice, cleanly, 4-4.5 minutes each).

**What batch-size tuning did *not* fully solve:** tried 16 (4m4s, marginal improvement over 8's 4m30s - ruled out batch size as the dominant time cost, it's the free tier's CPU) and 4 (more conservative memory, still crashed - a connection that died mid-request rather than a clean OOM traceback). Failing at all three tested batch sizes points at the free tier's resource ceiling under sustained load as the root cause, not a specific code bug to fix. Diagnosed by comparing curl (succeeded twice) against Node's native `fetch` and a real Playwright browser run (failed with `TypeError: Failed to fetch` / connection drop) - same operation, different client, different outcome, which is consistent with an intermittent server-side crash rather than a deterministic bug in any single client.

**Decision, made explicitly rather than silently accepted:** presented the user three options (accept as a documented limitation / pay for a small Render tier upgrade / re-architect ingestion as an async background job) and the user chose not to spend money. Landed on: document the limitation honestly in the README, add a keep-warm ping (`.github/workflows/keep-warm.yml`, pings `/health` every 10 minutes) to at least remove the *sleep-then-cold-start* tax on top of the CPU constraint, and leave the async-ingestion re-architecture as explicit future work rather than force a fix under a zero-budget constraint.

**Deliverable:** ✅ Live at zero cost: [app-woad-nu-48.vercel.app](https://app-woad-nu-48.vercel.app) (Vercel) → [veritas-ml-service.onrender.com](https://veritas-ml-service.onrender.com) (Render) → Neon Postgres. Everything except first-time ingestion of a brand-new paper is fast and reliable in production (verified: health checks, claim generation ~3.6s, quiz, publish/export). First-time ingestion of a new paper works but is inconsistent (confirmed working end-to-end at least twice with full valid output; also confirmed failing under repeated load) - a real, disclosed hosting-tier limitation, not a hidden one.

...
export function searchTopic(query: string, limit = 5): Promise<ArxivCandidate[]> {
  return request("/topics/search", z.array(ArxivCandidateSchema), {
    method: "POST",
...
export function createTopic(params: {
  ...
  return request("/topics", TopicSummarySchema, {
    method: "POST",
    body: JSON.stringify({
      query: params.query,
      max_papers: params.maxPapers ?? 5,
...
export function getTopic(topicId: string): Promise<Topic> {
  return request(`/topics/${topicId}`, TopicSchema);
}

export function listTopics(): Promise<TopicSummary[]> {
  return request("/topics", z.array(TopicSummarySchema));
}

export async function deleteTopic(topicId: string): Promise<void> {
  const response = await fetch(`${ML_SERVICE_URL}/topics/${topicId}`, { method: "DELETE" });
  if (!response.ok) {
    throw new Error(`DELETE /topics/${topicId} failed (${response.status})`);
  }
}

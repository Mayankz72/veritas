 ...
 * the free tier, not a bug (a keep-warm ping mitigates the sleep/cold-start
 * part but not the intrinsic embedding time). Give it real headroom rather
 * than a flaky local-sized timeout when pointed at a remote deployment.
 */
const isRemote = !!process.env.E2E_BASE_URL && !process.env.E2E_BASE_URL.includes("localhost");
// Measured locally: the same embedding work ranged from ~10s to 70s+ between runs on one
// laptop, so give a local run real headroom too.
const INGEST_TIMEOUT = isRemote ? 10 * 60_000 : 3 * 60_000;

test("ingest, generate claims, publish, and view the public page", async ({ page }) => {
  test.setTimeout(isRemote ? 12 * 60_000 : 5 * 60_000);

  await page.goto("/");
  await expect(page.getByRole("heading", { name: "Veritas" })).toBeVisible();

  // Single-paper ingest lives under the topic search on the home page.
  await page.getByText("Or open a single paper by arXiv ID").click();
